...
public class JMongoDataStore implements AdvancedDataStore {
    private static final Logger LOG = LoggerFactory.get(JMongoDataStore.class);
    ...
    public <T> WriteResult delete(String kind,final T entity) {
        return delete(kind,entity, getWriteConcern(entity));
    }
    ...
    public <T> WriteResult delete(String kind,final T entity, final WriteConcern wc) {
        return delete(kind,entity, new DeleteOptions().writeConcern(wc));
    }
    ...
    public <T> WriteResult delete(String kind,final T entity, final DeleteOptions options) {
        final T wrapped = ProxyHelper.unwrap(entity);
        if (wrapped instanceof Class<?>) {
            throw new MappingException("Did you mean to delete all documents? -- delete(ds.createQuery(???.class))");
        }
        try {
            return delete(kind,wrapped.getClass(), mapper.getId(wrapped), options);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
    ...
    public <T> WriteResult delete(final DBCollection dbColl,final Query<T> query, final DeleteOptions options) {
        if (query.getSortObject() != null || query.getOffset() != 0 || query.getLimit() > 0) {
            throw new QueryException("Delete does not allow sort/offset/limit query options.");
        }
        return dbColl.remove(query.getQueryObject(), enforceWriteConcern(options, query.getEntityClass()).getOptions());
    ...
    public <T, V> Query<T> find(String kind,final Class<T> clazz, final String property, final V value) {
        final Query<T> query = createQuery(kind,clazz);
        return query.filter(property, value);
    }
    ...
    public DBCollection getCollection(final String kind) {
        if (kind == null) {
            return null;
        }
    ...
    public synchronized  WriteResult save(DBCollection dbColl, DBObject document) {
        // try to do an update if there is a @Version field
        return dbColl.save(document);
    }
}
